    fn matvec_operation(rows: usize, cols: usize) -> TypedOperationSpec {
        TypedOperationSpec::new("test-matvec", OperationKind::Matvec, OperationRoute::CudaKernel)
            .with_input(
                TensorTypeSpec::new(NumericKind::F32, NumericKind::F32, [cols])
            ...
            .with_output(
                TensorTypeSpec::new(NumericKind::F32, NumericKind::F32, [rows])
                    .with_layout("contiguous"),
            )
    }
    ...
    fn operation_auto_optimize_renders_best_matvec_source() {
        ...
        assert_eq!(generated.optimization.problem.family(), "matvec-bf16-row-major");
    ...
    fn operation_problem_routes_gemm_search_and_source_generation() {
        let problem = InferenceKernelOptimizationProblem::from_operation(&gemm_operation(128, 128, 256))
    ...
    fn operation_auto_optimize_with_scorer_can_drive_gemm_generation() {
            ...
            auto_optimize_inference_kernel_with_scorer(&operation, config, |candidate| {
                ...
                if plan.reduce_unroll == 4 {
                    ...
                    SearchScore::measured(
                        1000.0 + f64::from(plan.tile.m + plan.tile.n + plan.tile.k),
                    )
                }
            })
        ...
        assert!(source.source.contains(&format!("pub fn {}(", source.symbol)));
    }
    ...
    fn operation_generation_reuses_selection_cache() {
        ...
        assert_eq!(first.candidate.artifact_key(), second.candidate.artifact_key());
        assert_eq!(first.source.symbol, second.source.symbol);
        assert!(second.source.source.contains(&format!(
            "pub fn {}(",
    ...
    fn operation_problem_rejects_unsupported_layouts_explicitly() {
        ...
        .with_input(
            TensorTypeSpec::new(NumericKind::F32, NumericKind::F32, [256])
                .with_layout("contiguous"),
        )
        .with_input(
        ...
        .with_output(
            TensorTypeSpec::new(NumericKind::F32, NumericKind::F32, [128])
                .with_layout("contiguous"),
        );
