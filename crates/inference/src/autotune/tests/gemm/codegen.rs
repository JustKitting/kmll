...
fn gemm_search_expands_tile_metadata_into_reduce_unroll_variants() {
    ...
    let group_top13 = candidates
        .iter()
        .find(|candidate| {
            candidate.schedule.transforms.iter().any(|transform| {
                matches!(
                    transform,
        ...
        .expect("GEMM search should expose a reduce group-top factor 13 descriptor");
    let group_top_plan =
        schedule_gemm_plan(&group_top13.schedule).expect("group-top candidate should plan");
    assert_eq!(group_top_plan.reduce_group_size(), 13);
    assert_eq!(
        group_top13.launch.kernel,
        "gemm_f32_bf16_tile_16x32x16_kg13"
    );
}
