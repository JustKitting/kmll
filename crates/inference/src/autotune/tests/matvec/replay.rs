...
#[test]
fn action_trace_replay_reconstructs_local_tiled_matvec_candidate() {
    let problem = MatvecSearchProblem::bf16_row_major(4096, 4096);
    let actions = vec![
    ...
    let candidate = replay_schedule_actions(&problem, &actions)
        .expect("local-tile matvec action trace should replay into candidate metadata");

    assert_eq!(candidate.family, "matvec-bf16-row-major");
    assert_eq!(candidate.action_trace, actions);
    assert_eq!(candidate.launch.kernel, "matvec_bf16_rows13");
    assert_eq!(schedule_rows_per_block(&candidate.schedule), Some(13));
    assert_eq!(
    ...
    let generated = MatvecRustCudaGenerator
        .source_for(&candidate)
        .expect("replayed local-tile matvec candidate should render source on demand");
