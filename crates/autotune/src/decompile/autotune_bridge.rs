...
use super::{
    KernelIrFunction, KernelIrModule, KernelIrOpKind, MemorySpace, SassSemanticPatternCategory,
    SassSymbol, recover_sass_patterns,
};
...
pub fn decompiled_autotune_operation(
    ...
    match shape {
        DecompiledAutotuneShape::MatvecBf16RowMajor { rows, cols } => {
            let evidence = matvec_bf16_row_major_evidence(function);
...
fn matvec_bf16_row_major_evidence(function: &KernelIrFunction) -> DecompiledAutotuneEvidence {
    let pattern_categories = typed_pattern_categories(function);
...
fn typed_pattern_categories(function: &KernelIrFunction) -> Vec<SassSemanticPatternCategory> {
    let module = KernelIrModule {
        target: None,
